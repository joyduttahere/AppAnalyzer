# BrandManager app.py
# Version 2.13 - Improved Prompting
...
- **Recommendation:** [Actionable 1 or 2 suggestions]"""
        ...
        review_texts = "\n".join([f"- {r['content']}" for r in top_critical_reviews])
        if any(name in selected_model for name in ["SmolLM"]):
            prompt = f"""<|im_start|>user
Analyze these critical reviews and identify the top 3 main problems:
{review_texts}
<|im_end|>
<|im_start|>assistant
Top 3 Problems:
1."""
            ...
            prompt = f"""Instruct: Analyze these critical reviews and identify the top 3 problems:
{review_texts}

List the top 3 problems in order of importance.
Output: Top 3 Problems:
1."""
       
        response_text = generate_response(models['reasoning_model'], models['reasoning_tokenizer'], prompt, max_length=300)
        # Validate and parse the local model's response
