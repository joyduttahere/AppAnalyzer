...
def get_models():
    ...
    if MODELS is None:
        ...
        classifier_model = AutoModelForSequenceClassification.from_pretrained("cardiffnlp/twitter-roberta-base-sentiment-latest").to(DEVICE)
        print("Classifier model loaded.")

        print(f"Loading Reasoning Model: mistralai/Mistral-7B-Instruct-v0.3...")
        reasoning_tokenizer = AutoTokenizer.from_pretrained("mistralai/Mistral-7B-Instruct-v0.3")
        reasoning_model = AutoModelForCausalLM.from_pretrained("mistralai/Mistral-7B-Instruct-v0.3", quantization_config=quantization_config, device_map="auto")
        print("Reasoning model loaded successfully.")
        
        MODELS = {
            'device': DEVICE, 'classifier_tokenizer': classifier_tokenizer, 'classifier_model': classifier_model,
            'reasoning_tokenizer': reasoning_tokenizer, 'reasoning_model': reasoning_model
        }
